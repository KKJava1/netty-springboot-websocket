...
import com.example.nettyspringbootwebsocket.annotations.*;
import com.example.nettyspringbootwebsocket.exception.WebsocketDeploymentException;
...
public class WebsocketServerEndpoint {
    ...
    /**
     * 在注册阶段（如 registerServerEndpoints 方法中），框架会动态解析服务端点类，找到标注了以下注解的方法：
     *
     * @HandshakeBefore：握手前执行
     * @OnOpen：客户端连接时触发
     * @OnMessage：收到消息时触发
     * @OnClose：连接关闭时触发
     * @OnError：发生错误时触发
     * @OnEvent：发生事件（如心跳事件）时触发
     */
